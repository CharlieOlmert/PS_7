...
polling_data <- master %>%
select(source, response, educ, ager, gender, race_eth, state_race_wave, wave, race_type, district_full, APPKAV, likely) %>%
...
#m2 <- lm(accuracy ~ race_eth, data = shiny_data) #let the user choose the variable to 

#summary(m2)

# For each election aggregate score for like percent black 

# You don't have to weight it Cause it's poll error 
# I could get both weighted and unweighted
# aggregate weights for a mean weight for the district. Higher weight = lots of adjustment cause sample not good
# does variation in the weights explain variation 
